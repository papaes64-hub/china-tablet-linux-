...
while IFS= read -r line; do
    if [[ "$line" =~ orientation[[:space:]]+changed:[[:space:]]+(bottom-up|normal|left-up|right-up) ]] ||        [[ "$line" =~ orientation:[[:space:]]*(bottom-up|normal|left-up|right-up) ]]; then
